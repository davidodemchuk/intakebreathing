...
function TtsNativeTab({ t, S, teamMembers, creators = [] }) {
  ...
  const analyzeWeek = async (w) => {
    ...
    try {
      let ibSettings = {};
      try { ibSettings = await fetchIBSettings() || {}; } catch (e) { console.warn("[tts] Could not load IB settings:", e.message); }
      const bc = ibSettings.ai_brand_context || {};
      const brandBlock = bc.positioning ? "CONTEXT: You are analyzing data for Intake Breathing — " + (bc.positioning || "") + ". " + (bc.product_description || "") : "CONTEXT: You are analyzing data for Intake Breathing, a magnetic external nasal dilator company focused on better breathing, sleep, and athletic performance.";
  ...
  const WowArrow = ({ current, previous, invert }) => {
    ...
    return <span style={{ fontSize: 9, fontWeight: 500, color, marginLeft: 4 }}>{change.dir === "up" ? "\u25B2" : "\u25BC"} {change.pct}%</span>;
  };
      ...
      return (
        <td style={{ ...cellStyle, padding: 0 }}>
          <input autoFocus type="number" step={step || "1"} value={editingValue} onChange={(e) => setEditingValue(e.target.value)}
            ...
            style={{ width: "100%", padding: "8px 10px", fontSize: 12, fontWeight: 500, border: "2px solid " + t.green, borderRadius: 4, background: t.inputBg, color: t.text, textAlign: align || "right", boxSizing: "border-box", outline: "none" }} />
        </td>
      );
    }
    ...
    <div>
        ...
        return (
          <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
            <div style={{ flex: 1, background: t.card, border: "1px solid " + t.border, borderRadius: 10, padding: "12px 16px", boxShadow: t.shadow }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 500, color: t.green, marginTop: 2 }}>{"$" + Math.round(gmv30).toLocaleString()}</div>
            ...
            <div style={{ flex: 1, background: t.card, border: "1px solid " + t.border, borderRadius: 10, padding: "12px 16px", boxShadow: t.shadow }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 500, color: t.blue, marginTop: 2 }}>{roas30 ? roas30 + "x" : "\u2014"}</div>
            ...
            <div style={{ flex: 1, background: t.card, border: "1px solid " + t.border, borderRadius: 10, padding: "12px 16px", boxShadow: t.shadow }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 500, color: t.text, marginTop: 2 }}>{videos30.toLocaleString()}</div>
            ...
            <div style={{ flex: 1, background: t.card, border: "1px solid " + t.border, borderRadius: 10, padding: "12px 16px", boxShadow: t.shadow }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 500, color: netRev30 >= 0 ? t.green : (t.red || "#ef4444"), marginTop: 2 }}>{"$" + Math.round(netRev30).toLocaleString()}</div>
          ...
          return (
            <div key={chart.key} style={{ background: t.card, border: "1px solid " + t.border, borderRadius: 12, padding: "16px 18px", boxShadow: t.shadow }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: t.text }}>{chart.label}</div>
                <div style={{ fontSize: 16, fontWeight: 500, color: chart.color }}>{chart.format(lastVal)}</div>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                ...
                {totalChange !== 0 ? <div style={{ fontSize: 11, fontWeight: 500, color: totalChange > 0 ? t.green : (t.red || "#ef4444") }}>{totalChange > 0 ? "+" : ""}{totalChange}% overall</div> : null}
              </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={newWeekQuick} style={{ padding: "6px 14px", borderRadius: 8, fontSize: 12, fontWeight: 500, border: "none", background: t.green, color: t.isLight ? "#fff" : "#000", cursor: "pointer" }}>+ New week</button>
        </div>
      </div>
      ...
      {showMilestoneForm ? (
        <div style={{ background: t.card, border: "2px solid " + t.blue + "40", borderRadius: 14, padding: 20, marginBottom: 16, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div style={{ fontSize: 15, fontWeight: 500, color: t.text }}>Add milestone</div>
      ...
      {showTargetForm ? (
        <div style={{ background: t.card, border: "2px solid " + (t.purple || "#8b6cc4") + "40", borderRadius: 14, padding: 20, marginBottom: 20, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <div style={{ fontSize: 15, fontWeight: 500, color: t.text }}>Monthly target</div>
      ...
      {showForm ? (
        <div style={{ background: t.card, border: "2px solid " + t.green + "40", borderRadius: 14, padding: 24, marginBottom: 20, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div style={{ fontSize: 16, fontWeight: 500, color: t.text }}>{editingRow ? "Edit week" : "New week entry"}</div>
              ...
              <div style={{ position: "absolute", top: "100%", left: 70, marginTop: 4, zIndex: 50, background: t.card, border: "1px solid " + t.border, borderRadius: 10, boxShadow: "0 8px 24px rgba(0,0,0,0.12)", minWidth: 240, overflow: "hidden", maxHeight: 300, overflowY: "auto" }}>
                {teamMembers.map(m => (
                  <div key={m.id} onClick={() => { setCurrentEnterer(m.id); try { localStorage.setItem("tts_enterer", m.id); } catch {} setEntererDropdownOpen(false); }} style={{ padding: "8px 12px", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 10, background: currentEnterer === m.id ? t.green + "10" : "transparent" }} onMouseEnter={(e) => { e.currentTarget.style.background = currentEnterer === m.id ? t.green + "15" : t.cardAlt; }} onMouseLeave={(e) => { e.currentTarget.style.background = currentEnterer === m.id ? t.green + "10" : "transparent"; }}>
                    {m.avatar_url ? <img src={m.avatar_url} alt="" style={{ width: 28, height: 28, borderRadius: 14, objectFit: "cover", flexShrink: 0 }} onError={(e) => { e.target.style.display = "none"; }} /> : <div style={{ width: 28, height: 28, borderRadius: 14, background: t.green + "15", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 500, color: t.green }}>{m.name?.[0]}</div>}
          ...
          <div style={{ display: "flex", gap: 24 }}>
            <div style={{ flex: 1 }}>
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 13, fontWeight: 500, color: t.text, marginBottom: 4 }}>Required weekly data</div>
              ...
              <div style={{ marginBottom: 16, padding: 12, background: t.cardAlt, borderRadius: 8 }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: t.text, marginBottom: 6 }}>GMV breakdown</div>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: t.orange || "#d4890a", marginBottom: 6 }}>Costs (affects ROAS + net revenue)</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  {inputField("Sample cost ($)", "sample_cost")}{inputField("Creator payments ($)", "creator_payments")}{inputField("Commission ($)", "tts_commission")}
                </div>
              </div>
              <div style={{ marginBottom: 16, padding: 12, background: t.cardAlt, borderRadius: 8 }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: t.text, marginBottom: 6 }}>Weekly content notes</div>
              ...
              <div style={{ marginBottom: 8 }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: t.textFaint, marginBottom: 6 }}>Creators (optional)</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
            ...
            <div style={{ width: 220, flexShrink: 0 }}>
              <div style={{ fontSize: 12, fontWeight: 500, color: t.green, marginBottom: 10 }}>Live calculations</div>
              <div style={{ background: t.cardAlt, borderRadius: 10, padding: 14 }}>
                {Object.entries(calc(formData)).map(([key, val]) => (
                  <div key={key} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid " + t.border + "40" }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 500, color: String(val).includes("\u2014") ? t.textFaint : t.text }}>{val}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 16 }}>
            ...
            <button onClick={saveForm} disabled={saving} style={{ padding: "8px 20px", borderRadius: 8, fontSize: 13, fontWeight: 500, border: "none", background: t.green, color: t.isLight ? "#fff" : "#000", cursor: "pointer", opacity: saving ? 0.6 : 1 }}>{saving ? "Saving..." : editingRow ? "Update" : "Save week"}</button>
          </div>
        </div>
      ) : null}
      ...
      {(() => {
        ...
        return <div style={{ marginBottom: 12, padding: "10px 14px", borderRadius: 8, background: t.isLight ? "#fef3cd" : "#332d1a", border: "1px solid " + (t.isLight ? "#f0d9a8" : "#4d4020"), fontSize: 12, color: t.isLight ? "#856404" : "#fbbf24", lineHeight: 1.6 }}><span style={{ fontWeight: 500 }}>Data gaps:</span> {issues.join(" \u00B7 ")}</div>;
      })()}
              ...
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0", fontSize: 12, tableLayout: "fixed", minWidth: Object.values(colWidths).reduce((s, w) => s + w, 0) }}>
                <thead style={{ position: "sticky", top: 0, zIndex: 10 }}>
                  <tr style={{ background: t.isLight ? "#e8e5dc" : "#222222", borderBottom: "2px solid " + t.border, height: 44 }}>
                    {(() => { const hs = { padding: "10px 12px", fontWeight: 500, color: t.text, borderBottom: "2px solid " + t.border, whiteSpace: "nowrap", textTransform: "uppercase", letterSpacing: "0.04em", fontSize: 10 }; const ho = weeks.some(w => Number(w.orders) > 0); return <>
                ...
                {(() => {
                    ...
                    const renderRow = (row, ri) => {
                      ...
                      if (row.type === "mt") {
                        ...
                        const mtb = "2px solid " + (t.isLight ? "#f0d9a8" : "#3d2f0f"); const mtc = t.isLight ? "#92600a" : "#fbbf24"; const mts = { padding: "8px 12px", fontSize: 11, fontWeight: 500, textAlign: "right", color: mtc, borderBottom: mtb };
                        return <tr key={"mt"+ri} style={{ background: t.isLight ? "#fef3e2" : "#2a1f0a" }}>
                          ...
                          <td style={{ ...mts, fontSize: 12, fontWeight: 500, color: t.green }}>{fmtDol(tg)}</td><td style={mts}>{fmtDol(sumW(ws,"organic_gmv"))}</td><td style={mts}>{fmtDol(sumW(ws,"paid_gmv"))}</td><td style={mts}>{fmtDol(ta)}</td><td style={mts}>{"\u2014"}</td>
                          <td style={{ ...mts, fontSize: 12, fontWeight: 500, color: roas !== "\u2014" && parseFloat(roas) >= 2 ? t.green : t.text }}>{roas}</td>
                          <td style={mts}>{ti > 0 ? "$"+(ta/(ti/1000)).toFixed(2) : "\u2014"}</td><td style={mts}>{tv > 0 ? "$"+Math.round(nr/tv).toLocaleString() : "\u2014"}</td>
                          <td style={{ ...mts, fontSize: 12, fontWeight: 500, color: nr >= 0 ? t.green : (t.red||"#ef4444") }}>{"$"+Math.round(nr).toLocaleString()}</td><td style={{ borderBottom: mtb }}></td><td style={{ borderBottom: mtb }}></td>
                        </tr>;
                      }
                      if (row.type === "qt") {
                        ...
                        const qtb = "3px solid " + (t.isLight ? "#b8d4c0" : "#2a4a32"); const qtc = t.isLight ? "#145a30" : "#4ade80"; const qts = { padding: "10px 12px", fontSize: 12, fontWeight: 500, textAlign: "right", color: qtc, borderBottom: qtb };
                      ...
                      if (row.type === "w") {
                        ...
                        result.push(
                          <tr key={"w-"+w.id} data-week={w.week_start} style={{ background: "transparent" }} onMouseEnter={(e) => { e.currentTarget.style.background = t.isLight ? "#ece9e0" : "#1e1e1e"; }} onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; }}>
                            ...
                            <EditableCell rowId={w.id} column="tts_gmv" value={w.tts_gmv} format={fmtDol} step="0.01" style={{ ...cs, fontWeight: 500, fontSize: 13, color: t.green }}>{pw ? <WowArrow current={w.tts_gmv} previous={pw.tts_gmv} /> : null}</EditableCell>
                            ...
                            <td style={{ ...cs, fontWeight: 500, fontSize: 13, color: c.roas !== "\u2014" && parseFloat(c.roas) >= 2 ? t.green : t.text }}>{c.roas}{pw ? <WowArrow current={Number(w.ad_spend) > 0 ? Number(w.tts_gmv) / Number(w.ad_spend) : 0} previous={Number(pw.ad_spend) > 0 ? Number(pw.tts_gmv) / Number(pw.ad_spend) : 0} /> : null}</td>
                            <td style={{ ...cs, color: t.textMuted }}>{c.cpm}</td>
                            <td style={cs}>{c.net_per_video}</td>
                            <td style={{ ...cs, fontWeight: 500, fontSize: 13, color: c.net_revenue.includes("-") ? (t.red || "#ef4444") : t.green }}>{c.net_revenue}{pw ? (() => { const p = calc(pw); const cVal = Number(String(c.net_revenue).replace(/[$,]/g, "")) || 0; const pVal = Number(String(p.net_revenue).replace(/[$,]/g, "")) || 0; return <WowArrow current={cVal} previous={pVal} />; })() : null}</td>
                          ...
                          expandedNotes === w.id ? <tr key={"detail-" + w.id}><td colSpan={99} style={{ padding: "12px 16px", background: t.cardAlt, borderBottom: "2px solid " + t.border }}>
                            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                              <div>
                                <div style={{ fontSize: 12, fontWeight: 500, color: t.text, marginBottom: 6 }}>Week notes</div>
                    ...
                    return qGroups.flatMap((group, gi) => {
                      ...
                      els.push(
                        <tbody key={"qtbody-" + gi} style={{ outline: "2px solid " + qColor, outlineOffset: "-1px" }}>
                          <tr style={{ background: qLabelBg }}><td colSpan={99} style={{ padding: "8px 14px", fontSize: 13, fontWeight: 500, color: qLabelColor, borderBottom: "1px solid " + qColor, letterSpacing: "-0.01em" }}>{group.q}</td></tr>
      ...
      {viewMode === "table" ? (
        <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px 0 4px", borderTop: "1px solid #1a1a1a", marginTop: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#00FEA9", display: "inline-block", boxShadow: "0 0 6px #00FEA9, 0 0 12px rgba(0,254,169,0.4)" }} />
            <span style={{ fontSize: 11, color: "#737373" }}>Generated by IB-Ai (live Anthropic API)</span>
          </div>
        </div>
      ) : null}
                ...
                <thead style={{ position: "sticky", top: 0, zIndex: 10 }}>
                  <tr style={{ background: t.isLight ? "#e8e5dc" : "#222222", borderBottom: "2px solid " + t.border, height: 44 }}>
                    {["Month", "SF Invites", "Requests", "Shipped", "Videos", "Impressions", "Orders", "GMV", "Ad Spend", "S/V", "ROAS", "CPM", "Net/video", "Net Revenue", "Weeks"].map((h, i) => (
                      <th key={i} style={{ padding: "10px 12px", textAlign: i === 0 || i === 14 ? "left" : "right", fontWeight: 500, color: t.text, borderBottom: "2px solid " + t.border, whiteSpace: "nowrap", textTransform: "uppercase", letterSpacing: "0.04em", fontSize: 11 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {(() => {
                    ...
                    const mkQTotal = (qLabel) => {
                      ...
                      return <tr key={"qt-" + qLabel} style={{ background: t.isLight ? "#c2dccb" : "#1a3322" }}>
                        <td style={{ padding: "10px 14px", fontSize: 13, fontWeight: 500, color: mqtc }}>{qLabel} total</td><td colSpan={4}></td>
                        <td style={{ padding: "10px 12px", textAlign: "right", fontSize: 12, fontWeight: 500, color: mqtc }}>{fmtNum(qMs.reduce((s, mm) => s + Number(mm.impressions || 0), 0))}</td>
                        <td style={{ padding: "10px 12px", textAlign: "right", fontSize: 12, fontWeight: 500, color: mqtc }}>{fmtNum(qMs.reduce((s, mm) => s + Number(mm.orders || 0), 0))}</td>
                        <td style={{ padding: "10px 12px", textAlign: "right", fontSize: 14, fontWeight: 500, color: mqtg }}>{fmtDol(qG)}</td>
                        <td style={{ padding: "10px 12px", textAlign: "right", fontSize: 12, fontWeight: 500, color: mqtc }}>{fmtDol(qA)}</td><td></td>
                        <td style={{ padding: "10px 12px", textAlign: "right", fontSize: 12, fontWeight: 500, color: mqtg }}>{qR}</td><td></td><td></td>
                        <td style={{ padding: "10px 12px", textAlign: "right", fontSize: 13, fontWeight: 500, color: qN >= 0 ? mqtg : "#ef4444" }}>${Math.round(qN).toLocaleString()}</td>
                    ...
                    monthly.forEach((m, mi) => {
                      ...
                      rows.push(
                        <tr key={"m-" + m.month} style={{ background: t.isLight ? "#fef3e2" : "#2a1f0a", borderBottom: "2px solid " + (t.isLight ? "#f0d9a8" : "#3d2f0f") }} onMouseEnter={(e) => { e.currentTarget.style.background = t.isLight ? "#fdecc8" : "#332808"; }} onMouseLeave={(e) => { e.currentTarget.style.background = t.isLight ? "#fef3e2" : "#2a1f0a"; }}>
                          <td style={{ padding: "10px 12px", fontWeight: 500, color: mc, whiteSpace: "nowrap" }}>
                            {monthLabel}{gmvPct != null ? <span style={{ marginLeft: 8, fontSize: 10, fontWeight: 500, color: gmvPct >= 100 ? t.green : gmvPct >= 75 ? t.orange : (t.red || "#ef4444") }}>{gmvPct}% to target</span> : null}
                          ...
                          <td style={{ padding: "10px 12px", textAlign: "right", fontWeight: 500, color: mc }}>{fmtNum(m.videos_posted)}</td>
                          ...
                          <td style={{ padding: "10px 12px", textAlign: "right", fontWeight: 500, fontSize: 13, color: t.green }}>{fmtDol(gmv)}{prevMonth ? (() => { const pG = Number(prevMonth.tts_gmv) || 0; if (pG === 0) return null; const pct = Math.round(((gmv - pG) / pG) * 100); if (pct === 0) return null; return <span style={{ fontSize: 9, fontWeight: 500, color: pct > 0 ? t.green : (t.red || "#ef4444"), marginLeft: 4 }}>{pct > 0 ? "\u25B2" : "\u25BC"}{Math.abs(pct)}%</span>; })() : null}</td>
                          ...
                          <td style={{ padding: "10px 12px", textAlign: "right", fontWeight: 500, color: roas !== "\u2014" && parseFloat(roas) >= 2 ? t.green : t.text }}>{roas}</td>
                          <td style={{ padding: "10px 12px", textAlign: "right", color: mc2 }}>{cpm}</td>
                          <td style={{ padding: "10px 12px", textAlign: "right", color: mc2 }}>{npv}</td>
                          <td style={{ padding: "10px 12px", textAlign: "right", fontWeight: 500, fontSize: 13, color: netRev >= 0 ? t.green : (t.red || "#ef4444") }}>${Math.round(netRev).toLocaleString()}</td>
